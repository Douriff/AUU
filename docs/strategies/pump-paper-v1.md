...
3. `buy_notional_1m >= 2 * sell_notional_1m` 且 `trade_count_1m >= 8`（不足则 `reason=momentum`）
4. 强 tape（仅纸面开仓）：`trade_count_1m >= min_trade_count_1m`（默认 **10**）且 `buy_notional_1m >= min_buy_sell_notional_ratio * sell_notional_1m`（默认 **2.5**）。字段是 `aggregate_tape` 的 60s 窗口（`trade_count_1m` / 买卖 SOL 名义），没有单独的买卖笔数。不足则 `reason=WEAK_TAPE`。出场不走这道门。
5. 入场含费冲击 `estimated_impact_gross_bps` **≤ `max_impact_bps`**（默认 **75**，80 硬顶下的缓冲）。**> 80** 一律拒单（`GROSS_IMPACT_HARD`），即使把参数抬到 80 以上。等于 80 且参数允许时可以过。
6. 无标签：`HONEYPOT` / `TAX_HIGH` / `SPREAD_TOO_WIDE`（若有外部打标）
7. 冷却：同 mint `cooldown_sec` 默认 **120s** 内不再开仓
...
min_trade_count_1m: 10              # paper entry; WEAK_TAPE below this
min_buy_sell_notional_ratio: 2.5   # buy_notional_1m / sell_notional_1m
...
版本：v1。只加参数不改事件名。 硬顶仍是含费冲击 **80**（入场默认缓冲 `max_impact_bps=75`）。纸面 Go 窗：`progress_bps [1200,6500]`，`take_profit_pct 0.10`，`stop_loss_pct 0.07`，`max_hold_sec 300`，`max_notional_sol 0.12`，`notional_pct_equity 0.005`。强 tape 默认 `trade_count_1m ≥ 10` 且买名义 ≥ **2.5×** 卖名义（`WEAK_TAPE`）。`strategy_autopaper`/`auto_paper_orders` default false。`liveEnabled` 默认 false。实盘名义硬顶 1.0 SOL 不变。
