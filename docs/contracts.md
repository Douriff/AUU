...
- `GET/PUT /api/v1/strategy/pump-paper-v1` — `auto_paper_orders` / `strategy_autopaper`（默认 false，无需重启）。纸面进程默认（AUU round 8b Go 窗）：`progress_bps [1500,6000]`、`take_profit_pct=0.06`、`stop_loss_pct=0.05`、`max_hold_sec=120`、`max_impact_bps=75`（含费 >80 硬拒）、`max_notional_sol=0.12`。入场动能默认 `min_trade_count_1m=10`、`min_buy_sell_ratio_1m=2.5`（仍 `reason=momentum`；不改 Go 门）。卖压出场默认 `sell_pressure_sec=5`、`sell_pressure_ratio=1.0`（`sell_notional_1m >= ratio * buy_notional_1m` 持续该秒数才 `reason=sell_pressure`；round 6 为 12s / 1.5，原先写死 30s / 2.0）。`PUT` 与 `PATCH` 都接受 `PumpPaperParamsPatch`。`liveEnabled` 默认 false；实盘名义硬顶 1.0 SOL。
