...
- **观察蒸馏**：`TraderWatchlist` → mock/`HeliusTraderReader`(default off) `TraderSnapshot` → `HabitEngine` → `DistillResult`；`apply-distill` 须确认后才 overlay 纸面参数。`copy_trade_enabled=false`；无钱包镜像。Journal 胜率只吃自有纸面。
