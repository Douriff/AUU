...
- **Live adapter（dark）**：`liveEnabled` 默认 false；LOCAL-ONLY `secrets/live-keypair.json`（gitignored）+ 二次确认 + 独立 `LiveLimits`（1 / 0.045 / 10）才离开 `LIVE_DISABLED`。Health：`keypairMounted` bool + `pubkeyShort`，永不返回 secret。见 `docs/adapters/pumpfun-live-local-signer-v0.md`、`docs/viz/live-ui-gates-v0.md`。本轮不发链上 tx。
