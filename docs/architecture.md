...
- **Live adapter（dark）**：`LiveDisabled` 默认开；`LocalSigner` 只读本机 `AUU_SOLANA_KEYPAIR_PATH`；缺 limits / keypair 无法 arm。见 `docs/adapters/pumpfun-live-local-signer-v0.md`。本轮不发链上 tx。
